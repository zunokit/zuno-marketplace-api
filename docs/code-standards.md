...
All services must support version injection via Go linker flags (`-ldflags`):

**Makefile Configuration**:

```makefile
# Version information for CI/CD release tracking
...
// Default values used when building locally without Makefile
var (
    Version   = "dev"
    BuildTime = "unknown"
)

// getBuildVersion returns the version string for observability
func getBuildVersion() string {
    return Version
}

func main() {
    // Log version at startup for debugging
    log.Printf("Starting service Version: %s BuildTime: %s", Version, BuildTime)

    // Pass version to observability (Sentry)
    obs.Init(cfg.Sentry.DSN, cfg.Sentry.Environment, "auth-service", getBuildVersion(), sampleRate)

    // Optionally expose via health endpoint
...
The `git describe --tags --always --dirty` command produces:

| Git State | Version Output | Example |
|-----------|----------------|---------|
| Tagged commit | `v1.0.0` | Clean release |
| Commits after tag | `v1.0.0-5-g123abcd` | 5 commits after v1.0.0 |
| No tags, on branch | `feature-add-sentry-123abcd` | Branch-based |
| Uncommitted changes | `*-dirty` suffix | Any state + dirty |
| No git available | `dev` | Fallback |

### CI/CD Integration

**Sentry Release Tracking**:

Version is automatically passed to Sentry for release-to-error correlation:

```go
// Sentry init with version
obs.Init(
    cfg.Sentry.DSN,
    cfg.Sentry.Environment,
    "auth-service",
    getBuildVersion(),  // Maps to Sentry release
    obsTrace.GetTracesSampleRate(cfg.Sentry.Environment),
)
```

**GitHub Actions** creates Sentry releases:
...
- name: Create Sentry Release
  run: |
    ...
    sentry-cli releases new "$VERSION" --org ${{ env.SENTRY_ORG }}
...
1. **Release Tracking**: Correlate errors with specific deployed versions
2. **Debugging**: Know exactly which commit is running in production
3. **Rollback Support**: Identify and revert to known-good versions
4. **Audit Trail**: Build timestamps for compliance
5. **CI/CD Automation**: Automated release creation and deploy notifications
